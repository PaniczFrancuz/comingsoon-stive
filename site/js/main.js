$(function() {
    ...
    function checkWhenEvent(){
      //Global var
      width = $(window).width();
      height = $(window).height();
      scrollCheck = $( window ).scrollTop();
      // dev
      $('.wyso').html("W " + height);    // Display the height
      $('.szero').html("S " + width );   // Display the width


      $('.full--height').css("height", height);
    };

    //  Wywołanie funkcji wyżej (checkWhenEvent)
    $(document).ready(checkWhenEvent);
    $(window).resize(checkWhenEvent);
    // $(window).scroll(checkWhenEvent);
